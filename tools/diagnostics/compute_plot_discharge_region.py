...
size_array = 3 + 1.0 * np.abs(kges_to_plot)
#
...
#*sc = ax.scatter(lons_to_plot, lats_to_plot, c=kges_to_plot,
#*                cmap='coolwarm', s=20, transform=ccrs.PlateCarree())
sc = ax.scatter(
...
for region, coords in regions.items():
    ...
    ax.add_patch(plt.Rectangle((lon_min, lat_min), lon_max - lon_min, lat_max - lat_min,
                               edgecolor='red', facecolor='none', linewidth=1.0, transform=ccrs.PlateCarree()))

plt.colorbar(sc, ax=ax, label='ΔKGE (GLA - CTL)')
plt.tight_layout()
plt.savefig(f'kge_map_regions_discharge_{expids[1]}-{expids[0]}.pdf')


#*# 4) Plot with Cartopy
#*# 5) Scatter with discrete colormap, alpha array, and size array

#*
#*# 6) Colorbar and final touches
#*plt.colorbar(sc, ax=ax, label='ΔKGE (Exp - Ctl)')
#*plt.title('Discrete KGE Map with Size & Alpha by Magnitude')
#*plt.tight_layout()
